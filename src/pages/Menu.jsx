...
const Menu = () => {
    ...
    // BOGO state — bogoOpen = true/false; category chosen inside the modal
    ...
    useEffect(() => {
        ...
        // Handle scrollTo from craving section navigation
        if (location.state?.scrollTo) {
            const targetId = location.state.scrollTo;
            // Scroll after a short delay to ensure sections are rendered
    ...
    useEffect(() => {
        let slowTimer = setTimeout(() => setApiSlow(true), 5000); // show "warming up" after 5s
    ...
    // Merge DB items with static data
            ...
            const mergeArr = (arr, catType, subCatType = null) => {
                const updatedStatic = arr.map(staticItem => {
                    ...
                    if (live) {
                        ...
                        const hasCustomImg = img && typeof img === 'string' && (img.startsWith('http') || img.startsWith('data:') || img.startsWith('/'));
                        return {
                            ...staticItem,
                            ...
                            image: hasCustomImg ? img : staticItem.image
                        };
                    }
                    return staticItem;
                });
                ...
                const newLive = dbItems.filter(dbItem => {
                    ...
                    return {
                        ...
                        image: hasCustomImg ? img : 'https://images.unsplash.com/photo-1541745537411-b8046f4d5092?w=400&q=70',
    ...
    // BOGO pizza pools — use flexible matching (case-insensitive, partial)
    const bogoPizzasByCategory = React.useMemo(() => {
        const found = (keyword) => {
            ...
            if (!cat) return [];
            // cat.data is the full pizza-category object {category, id, items}
        ...
        <div key={category.id} id={category.id} className="menu-section" ref={el => sectionRefs.current[category.id] = el}>
            ...
            <div className="product-grid">
                {(category.items || []).map(pizza => {
                    ...
                    const imgSrc = pizza.image && typeof pizza.image === 'string' &&
                        (pizza.image.startsWith('http') || pizza.image.startsWith('data:'))
                        ? pizza.image : pizza.image?.startsWith('/uploads')
                            ? `${API_URL}${pizza.image}` : pizza.image
                                ? `/images/menu/${pizza.image}` : 'https://images.unsplash.com/photo-1541745537411-b8046f4d5092?w=400&q=70';

                    // Build size list: price can be an object {small, medium, large} or a flat number
                    ...
                    const sizeDisplay = { small: 'S', medium: 'M', large: 'L', regular: 'Regular' };
                    ...
                    return (
                        <div key={pizza.id || pizza.name} className="menu-item-card card">
                            <div className="item-image-container">
                                ...
                                <img src={imgSrc} onError={e => { e.target.src = 'https://images.unsplash.com/photo-1541745537411-b8046f4d5092?w=400&q=70'; }} alt={pizza.name} className="item-image" loading="lazy" />
                            </div>
                            <div className="item-content">
                                ...
                                <div className="item-bottom-row">
                                    {sizeLabels.length > 0 ? (
                                        ...
                                        <>
                                            <span className="price">₹{flatPrice}</span>
                                            <button className="add-btn" onClick={() => handleAddToCart(pizza)}>
                                                Add +
                                            </button>
                                        </>
                                    )}
        ...
        <div key={id} id={id} className="menu-section" ref={el => sectionRefs.current[id] = el}>
            ...
            <div className="product-grid">
                {(items || []).map(item => {
                    ...
                    const isSupremeBogo = itemName.toLowerCase().includes('supreme');
                    const displayPrice = getDisplayPrice(item.price);
                    const imgSrc = item.image && typeof item.image === 'string' &&
                        (item.image.startsWith('http') || item.image.startsWith('data:'))
                        ? item.image : item.image?.startsWith('/uploads')
                            ? `${API_URL}${item.image}` : item.image
                                ? `/images/menu/${item.image}` : 'https://images.unsplash.com/photo-1541745537411-b8046f4d5092?w=400&q=70';
                    return (
                        <div key={item.id || itemName} className="menu-item-card card">
                            <div className="item-image-container">
                                {isBestseller && <span className="bestseller-badge">⭐ Bestseller</span>}
                                {isBogo && <span className="bestseller-badge" style={{ background: 'linear-gradient(135deg,#1B5E20,#4CAF50)' }}>🎁 BOGO</span>}
                                <img src={imgSrc} onError={e => { e.target.src = 'https://images.unsplash.com/photo-1541745537411-b8046f4d5092?w=400&q=70'; }} alt={itemName} className="item-image" loading="lazy" />
                            </div>
                            <div className="item-content">
                                <h4>{itemName}</h4>
                                ...
                                <div className="item-bottom-row">
                                    ...
                                    {isBogo ? (
                                        <button className="add-btn" onClick={() => openBogo()}>
                                            Select +
                                        </button>
                                    ) : (
    ...
    // ── BOGO handlers ─────────────────────────────────────────────────────────
    const openBogo = () => { setBogoOpen(true); setBogoSel({ category: null, size: 'medium', pizza1: null, pizza2: null }); };
    const closeBogo = () => { setBogoOpen(false); setBogoSel({ category: null, size: 'medium', pizza1: null, pizza2: null }); };
    ...
    return (
        <div className="menu-page animate-fade-in">
            ...
            {/* ── BOGO Modal — Category → Size → Pizza ── */}
            {bogoOpen && (() => {
                const cat = bogoSel.category; // null | 'Deluxe Veg' | 'Supreme Veg'
                ...
                return (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '10px 15px', paddingTop: '88px' }} onClick={closeBogo}>
                        <div style={{ width: '100%', maxWidth: '780px', maxHeight: '88vh', background: '#fff', borderRadius: '24px', overflow: 'hidden', display: 'flex', flexDirection: 'column', animation: 'popIn 0.3s' }} onClick={e => e.stopPropagation()}>
                            ...
                            <div style={{ background: grad, padding: '18px 24px', color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                <button onClick={closeBogo} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: '#fff', width: '34px', height: '34px', borderRadius: '50%', cursor: 'pointer', fontSize: '1.3rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
                            </div>
                            ...
                            <div style={{ padding: '20px', overflowY: 'auto', flex: 1 }}>
                                        ...
                                        {['Deluxe Veg', 'Supreme Veg'].map(c => (
                                            <button key={c} onClick={() => setBogoSel(p => ({ ...p, category: c, pizza1: null, pizza2: null }))}
                                                style={{
                                                    ...
                                                    border: `2.5px solid ${bogoSel.category === c ? primary : '#DDD'}`,
                                                    background: bogoSel.category === c ? bg : '#FAFAFA',
                                                    color: bogoSel.category === c ? primary : '#666',
                                                    transition: 'all 0.2s', fontFamily: 'inherit'
                                                }}>
                                                {c === 'Deluxe Veg' ? '⭐ Deluxe Veg' : '👑 Supreme Veg'}
                                                <div style={{ fontSize: '0.72rem', fontWeight: 500, marginTop: '4px', opacity: 0.75 }}>
                                                    {c === 'Deluxe Veg' ? 'Rs.180–520' : 'Rs.210+'}
                                                </div>
                                            </button>
                                        ))}
                                    </div>
                                ...
                                {/* ── STEP 2: Choose Size (only when category chosen) ─ */}
                                {cat && (
                                    <div style={{ marginBottom: '20px' }}>
                                        <div style={{ fontWeight: 800, fontSize: '0.75rem', color: '#888', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '10px' }}>
                                            Step 2 — Choose Size
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                                            {['medium', 'large'].map(sz => (
                                                <button key={sz} onClick={() => setBogoSel(p => ({ ...p, size: sz, pizza1: null, pizza2: null }))}
                                                    style={{ padding: '14px', border: `2px solid ${bogoSel.size === sz ? primary : '#EEE'}`, background: bogoSel.size === sz ? bg : '#f9f9f9', borderRadius: '14px', fontWeight: 800, cursor: 'pointer', fontSize: '0.95rem', transition: 'all 0.2s', fontFamily: 'inherit' }}>
                                ...
                                {/* ── STEP 3: Pick 2 Pizzas (only when category chosen) ─ */}
                                {cat && (
                                    <div>
                                        <div style={{ fontWeight: 800, fontSize: '0.75rem', color: '#888', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '10px' }}>
                                            Step 3 — Pick Your 2 Pizzas
                                        </div>
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                                            {[1, 2].map(slot => {
                                                ...
                                                return (
                                                    <div key={slot}>
                                                        <div style={{ fontWeight: 700, fontSize: '0.82rem', marginBottom: '8px', color: sel ? primary : '#555' }}>
                                                            Pizza {slot} {sel ? `— ✓ ${sel.name}` : '(Select below)'}
                                                        </div>
                                                        <div style={{ maxHeight: '240px', overflowY: 'auto', border: `2px solid ${sel ? primary : '#EEE'}`, borderRadius: '14px', padding: '6px' }}>
                                                            {pizzaPool.length === 0 ? (
                                                                <div style={{ padding: '20px', textAlign: 'center', color: '#aaa', fontSize: '0.85rem' }}>Pizzas loading…</div>
                                                            ) : pizzaPool.map(p => {
                                                                ...
                                                                return (
                                                                    <div key={p.id || p.name}
                                                                        onClick={() => !isO && (slot === 1 ? setBogoSel(v => ({ ...v, pizza1: isMe ? null : p })) : setBogoSel(v => ({ ...v, pizza2: isMe ? null : p })))}
                                                                        style={{ padding: '9px 11px', marginBottom: '5px', borderRadius: '10px', border: `1.5px solid ${isMe ? primary : 'transparent'}`, background: isMe ? bg : isO ? '#f5f5f5' : '#fff', opacity: isO ? 0.4 : 1, cursor: isO ? 'not-allowed' : 'pointer', transition: 'all 0.15s' }}>
                                                                        <div style={{ fontWeight: 700, fontSize: '0.85rem', color: '#222' }}>{p.name}</div>
                                                                        <div style={{ fontSize: '0.75rem', color: primary, fontWeight: 700, marginTop: '2px' }}>Rs.{pPrice} ({bogoSel.size})</div>
                                                                    </div>
                                                                );
                                                            })}
                                                        </div>
                                ...
                                {!cat && (
                                    <div style={{ textAlign: 'center', padding: '20px 0', color: '#aaa', fontSize: '0.9rem', fontWeight: 600 }}>
                                        ☝️ First select a pizza category above
                                    </div>
                                )}
                            </div>
                                ...
                                {bogoSel.pizza1 && bogoSel.pizza2 ? (
                                    <div>
                                        <div style={{ fontSize: '0.8rem', color: '#666', marginBottom: '10px', textAlign: 'center' }}>
                                            {bogoSel.pizza1.name} + {bogoSel.pizza2.name} ({bogoSel.size}) • Pay only{' '}
                                            <strong style={{ color: primary }}>Rs.{oP}</strong>
                                        </div>
